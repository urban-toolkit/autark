 ...
 * specified by config.attributes.axis[0]. Selection emits and receives *feature
 ...
 */
export class Histogram extends PlotBaseInteractive {
    ...
    public render(): void {
        ...
        const axisAttr = this.renderAxisAttributes[0];
        const values: BinDatum[] = this._data.map((d, i) => ({
            value: Number(d ? valueAtPath(d, axisAttr) ?? 0 : 0),
            index: i,
        }));
        ...
        const yLabel = this._axisLabels[1] ?? 'Count';
        yAxisSel
    ...
    protected override renderSelection(): void {
        ...
        d3.select(this._div).selectAll('.autkMark').style('fill', function (_d: unknown, binIdx: number) {
            ...
            const fullySelected = !!bin && bin.length > 0 && bin.every((d) => selectedSet.has(d.index));
    ...
    protected override clickEvent(): void {
        ...
        svgs.each(function (_d, binIdx: number) {
            d3.select(this).on('click', function () {
                ...
                const allSelected = binFeatureIds.length > 0 && binFeatureIds.every((id) => current.includes(id));
    ...
    protected override brushEvent(): void {
    ...
    protected override brushXEvent(): void {
        const brushable = d3.select(this._div).selectAll<SVGGElement, unknown>('.autkBrushable');
        const marksGroup = d3.select(this._div).selectAll<SVGGElement, unknown>('.autkMarksGroup');
    ...
    protected override brushYEvent(): void {
        const brushable = d3.select(this._div).selectAll<SVGGElement, unknown>('.autkBrushable');
        const marksGroup = d3.select(this._div).selectAll<SVGGElement, unknown>('.autkMarksGroup');
        const innerHeight = this._height - this._margins.top - this._margins.bottom;
        const plot = this;
