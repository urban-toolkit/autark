...
export class ParallelCoordinates extends PlotD3 {
    ...
    public async draw(): Promise<void> {
        ...
        dimensions.forEach((dim) => {
            ...
            const sampleValues = this.data.slice(0, 100).map(d => d ? d[dim] : null).filter(v => v !== null && v !== undefined);
            const isNumerical = sampleValues.every(v => !isNaN(Number(v)));
            
            if (isNumerical) {
                // Numerical scale
                this.dimensionTypes.set(dim, 'numerical');
                const extent = d3.extent(this.data, (d) => d ? +d[dim] || 0 : 0) as [number, number];
