/**
 * Visual regression test for the osm-layers-api-niteroi gallery example.
 * Overpass API responses are replayed from a local HAR file; run
 * `make test-update cache APP=gallery OPEN=/src/autk-map/osm-layers-api-niteroi.html` to re-record.
 */
...
test('osm-layers-api-niteroi', async ({ page }) => {
    ...
    await routeOverpassHar(page, path.join(__dirname, '../../data/osm-layers-api-niteroi.har'), false);
